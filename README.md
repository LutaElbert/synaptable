...
Browser storage can be cleared by the user, browser, device policy, or storage pressure. Use **Project backup and restore** to inspect approximate storage status, request persistent local storage where supported, create size-bounded local checkpoints, and download a portable `.synaptable` file before changing devices or clearing site data. Version 1–5 project files are migrated automatically to the current schema version 6. See [PRIVACY.md](./PRIVACY.md).
